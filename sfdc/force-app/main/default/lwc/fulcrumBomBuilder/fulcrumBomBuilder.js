...
import searchBomCatalog from '@salesforce/apex/FulcrumController.searchBomCatalog';
import analyzeBom from '@salesforce/apex/FulcrumController.analyzeBom';
import saveBomLines from '@salesforce/apex/FulcrumController.saveBomLines';
import getBomLines from '@salesforce/apex/FulcrumController.getBomLines';
...
const MANUFACTURERS = ['All', 'Cisco', 'HPE', 'Juniper', 'Palo Alto Networks', 'Fortinet', 'VMware'];
const CATEGORIES = ['All', 'Hardware', 'Software', 'Services', 'Support', 'Cloud', 'Subscription', 'Switches', 'Routers', 'Security', 'Wireless', 'Firewalls', 'Accessories'];
...
export default class FulcrumBomBuilder extends LightningElement {
  @api recordId;
    ...
    try {
      ...
      if (lines && lines.length > 0) {
        this.bomLines = lines.map(l => ({
          ...
          recommendedMarginPct: l.Recommended_Margin_Pct__c != null ? n(l.Recommended_Margin_Pct__c) : null,
  ...
  get hasBomLines() { return this.bomLines.length > 0; }
  get isEmpty() { return this.bomLines.length === 0; }
  get showEmptyState() { return this.bomLines.length === 0; }
  get lineCount() { return this.bomLines.length; }
  get hasAnalysis() { return this.analysis != null; }
  get hasAnalysisWithRecs() {
    return this.analysis != null && this.analysis.lines && this.analysis.lines.length > 0;
  }
  ...
  get tableRows() {
    return this.bomLines.map(line => {
      const unitPrice = line.marginPct >= 100 ? line.unitCost * 10 : line.unitCost / (1 - line.marginPct / 100);
      ...
      const recDeltaDisplay = hasDelta ? (delta >= 0 ? '+' : '') + delta.toFixed(1) : '';
      const recDeltaClass = !hasDelta ? 'delta-badge neutral' : delta > 0.5 ? 'delta-badge positive' : delta < -0.5 ? 'delta-badge negative' : 'delta-badge neutral';
      ...
      return {
        ...
        deltaDisplay: delta != null ? (delta >= 0 ? '+' : '') + delta.toFixed(1) + '%' : '\u2014',
        deltaClass: delta == null ? 'delta neutral' : delta > 0.5 ? 'delta positive' : delta < -0.5 ? 'delta negative' : 'delta neutral',
        marginHealthClass: delta == null ? '' : delta > 2 ? 'health-red' : delta > 0 ? 'health-yellow' : 'health-green',
        categoryClass: 'cat-badge cat-' + (line.category || 'Hardware').toLowerCase().replace(/\s+/g, '-')
      };
    });
  }
  ...
  get totalPrice() {
    return this.bomLines.reduce((s, l) => {
      const up = l.marginPct >= 100 ? l.unitCost * 10 : l.unitCost / (1 - l.marginPct / 100);
      return s + up * l.quantity;
    }, 0);
  }
  get totalGP() { return this.totalPrice - this.totalCost; }
  get blendedMarginPct() {
    return this.totalPrice > 0 ? ((this.totalPrice - this.totalCost) / this.totalPrice) * 100 : 0;
  }
  get totalCostDisplay() { return fmt$(this.totalCost); }
  get totalPriceDisplay() { return fmt$(this.totalPrice); }
  get totalGPDisplay() { return fmt$(this.totalGP); }
  get blendedMarginDisplay() { return fmtPct(this.blendedMarginPct); }
  ...
  get formattedInsights() {
    const raw = this.analysisResult?.insights || this.analysis?.recommendations?.insights || [];
  ...
  get marginGap() {
    ...
    return this.analysis.recommendations.suggestedBlendedMargin - this.blendedMarginPct;
  }
  ...
  async doSearch() {
    ...
    try {
      const mfg = this.searchManufacturer === 'All' ? null : this.searchManufacturer;
  ...
  handleSelectProduct(e) {
    ...
    if (product) {
      ...
      setTimeout(() => {
        const rows = this.template.querySelectorAll('[role="row"]:not(.table-header):not(.table-totals)');
        if (rows.length > 0) {
  ...
  handleSearchBlur() {
    ...
    setTimeout(() => { this.showSearchResults = false; }, 250);
  }
    ...
    try {
      ...
      if (this.analysis.lines) {
        this.bomLines = this.bomLines.map((l, i) => {
          ...
          if (analyzed) {
            return { ...l, recommendedMarginPct: analyzed.recommendedMarginPct };
          }
          return l;
        });
      }
  ...
  async handleSave() {
    ...
    try {
      const lines = this.bomLines.map((l, i) => ({
        ...
        quantity: l.quantity,
        unitCost: l.unitCost,
        unitPrice: l.marginPct >= 100 ? l.unitCost * 10 : l.unitCost / (1 - l.marginPct / 100),
        listPrice: l.listPrice || 0,
        extendedCost: l.unitCost * l.quantity,
        extendedPrice: (l.marginPct >= 100 ? l.unitCost * 10 : l.unitCost / (1 - l.marginPct / 100)) * l.quantity,
        marginPct: l.marginPct / 100,
        recommendedMarginPct: l.recommendedMarginPct != null ? l.recommendedMarginPct / 100 : null,
      ...
      this.dispatchEvent(new ShowToastEvent({
        title: 'BOM Saved',
        message: this.bomLines.length + ' line items saved to this opportunity',
        variant: 'success'
      }));
    } catch (e) {
      this.dispatchEvent(new ShowToastEvent({
  ...
  handleCsvFile(event) {
    ...
    reader.onload = () => {
      try {
        ...
        if (lines.length < 2) {
          this.dispatchEvent(new ShowToastEvent({ title: 'CSV Import', message: 'No data rows found in CSV', variant: 'warning' }));
          return;
        }
        ...
        const partCol = colIndex(['partnumber', 'partno', 'part', 'sku', 'productnumber']);
        const descCol = colIndex(['description', 'desc', 'name', 'productname']);
        ...
        if (newLines.length > 0) {
          ...
          this.dispatchEvent(new ShowToastEvent({ title: 'CSV Import', message: 'No valid data rows found', variant: 'warning' }));
        }
      } catch (err) {
        this.dispatchEvent(new ShowToastEvent({ title: 'CSV Import Error', message: err.message || 'Failed to parse CSV', variant: 'error' }));
      }
